...
class UIToolbar(tk.Frame, SimulationListener):
    def __init__(self, simulation: Simulation, master, **kwargs):
        ...
        self.start_stop_btn.bind(
            "<ButtonRelease-1>", lambda _: self.on_start_stop_click()
        )
        ...
        self.speed_frame = tk.Frame(
            self.controls, background="#ffffff", highlightthickness=0
        )
        self.speed_frame.grid(row=0, column=1, sticky="nsew")
        self.speed_down_btn_img = Image.open(resource_path("resources/minus-sign.png")).resize((16, 16), Image.Resampling.LANCZOS)
        self.speed_down_btn_img = ImageTk.PhotoImage(self.speed_down_btn_img)
        self.speed_up_btn_img = Image.open(resource_path("resources/plus-sign.png")).resize((16, 16), Image.Resampling.LANCZOS)
        self.speed_up_btn_img = ImageTk.PhotoImage(self.speed_up_btn_img)
        self.speed_down_btn = tk.Button(
            self.speed_frame,
            image=self.speed_down_btn_img,
            cursor="hand2",
            anchor="center",
            borderwidth=0,
            highlightthickness=0,
        ...
        self.speed_down_btn.bind(
            "<ButtonRelease-1>", lambda _: self.on_speed_change(faster=False)
        )
        ...
        self.speed_up_btn.bind(
            "<ButtonRelease-1>", lambda _: self.on_speed_change(faster=True)
        )
        ...
        if self.running:
            ...
            def auto_sim(curr_id):
                if curr_id == self.running_id:
                    ...
                    self.after(
                        int(round(1000 / self.speed, 0)),
                        lambda send_id=curr_id: auto_sim(send_id),
                    )
...
class UICanvas(tk.Canvas, SimulationListener):
    ...
    def on_scroll(self, event):
        ...
        self.offset_x = round(world_x * self.zoom - event.x)
        self.offset_y = round(world_y * self.zoom - event.y)
        self.redraw()
